...
class ParabolicModel(Model):
    ...
    def train(self):
        x_3 = self.x_2 * self.x_train
        x_4 = x_3 * self.x_train
        x_2_y = self.x_2 * self.y_train
        x_3_avg = np.average(x_3)
        x_4_avg = np.average(x_4)
        x_2_y_avg = np.average(x_2_y)
        eq_left_side = np.array([[1, self.x_avg, self.x_2_avg], [self.x_avg, self.x_2_avg, x_3_avg], [self.x_2_avg, x_3_avg, x_4_avg]])
        eq_right_side = np.array([self.y_avg, self.x_y_avg, x_2_y_avg])
    ...
    def test(self):
        y_t = self.a + self.b * self.x_test + self.c * (self.x_test * self.x_test)
        e = y_t - self.y_test
    ...
    def calculate_mse(self, x, y):
        y_t = self.a + self.b * x + self.c * (x * x)
